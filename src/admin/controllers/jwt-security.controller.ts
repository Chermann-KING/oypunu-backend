...
import { JwtSecretValidatorService, JwtSecretValidationResult } from '../../auth/security/jwt-secret-validator.service';
...
export class JwtSecurityController {
    ...
    schema: {
      ...
      properties: {
        secret: {
          ...
          example: 'MySecureJwtSecret123!@#$%^&*()_+ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefg'
        }
      },
      required: ['secret']
    }
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        strength: { type: 'string', enum: ['weak', 'medium', 'good', 'excellent'] },
  ...
  async validateSecret(@Body() body: { secret: string }): Promise<JwtSecretValidationResult & { timestamp: string }> {
    if (!body.secret) {
      throw new Error('Secret JWT requis pour validation');
    }

    const validationResult = this.jwtSecretValidator.validateJwtSecret(body.secret);
  ...
  async generateSecret(@Body() body: { length?: number } = {}): Promise<{
    ...
    if (length < 32 || length > 128) {
      throw new Error('Longueur du secret doit être entre 32 et 128 caractères');
    }
  ...
  @ApiOperation({ 
    ...
    description: 'Analyse la sécurité du secret JWT actuellement configuré (sans révéler le secret)'
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({ 
    ...
    description: 'Documentation des standards de sécurité appliqués pour les secrets JWT'
  })
  @ApiResponse({
    status: 200,
