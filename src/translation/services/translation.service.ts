...
import { TranslationGroup, TranslationGroupDocument } from '../schemas/translation-group.schema';
import { TrainingData, TrainingDataDocument } from '../schemas/training-data.schema';
...
export class TranslationService {
  constructor(
    ...
    @InjectModel(TranslationGroup.name) private translationGroupModel: Model<TranslationGroupDocument>,
    @InjectModel(TrainingData.name) private trainingDataModel: Model<TrainingDataDocument>,
  ...
  async getAvailableLanguages(wordId: string): Promise<AvailableLanguageDto[]> {
    ...
    if (word.translations.some(t => t.translationGroupId)) {
      const groupId = word.translations.find(t => t.translationGroupId)?.translationGroupId;
      if (groupId) {
        const relatedWords = await this.wordModel.find({
          'translations.translationGroupId': groupId
        });
        groupLanguages = relatedWords.map(w => w.language || 'fr').filter(Boolean);
      }
    }
    ...
    const languageStats = await Promise.all(
      allLanguages.map(async (lang) => {
        const translationsToLang = word.translations.filter(t => t.language === lang);
        const avgQuality = translationsToLang.length > 0 
          ? translationsToLang.reduce((sum, t) => sum + (t.votes || 0), 0) / translationsToLang.length
          : 0;
    ...
    return languageStats.sort((a, b) => b.translationCount - a.translationCount);
  }

  /**
   * Récupère la traduction d'un mot vers une langue spécifique
   */
  async getTranslation(wordId: string, targetLanguage: string): Promise<TranslationDto[]> {
    ...
    const translations = word.translations.filter(t => t.language === targetLanguage);
  ...
  async createTranslation(
    ...
    const sourceWord = await this.wordModel.findById(createTranslationDto.sourceWordId);
    if (!sourceWord) {
    ...
    if (similarWords.length > 0) {
      ...
      const similarity = this.similarityService.calculateSimilarity(sourceWord, bestMatch.word);

      // Utiliser l'apprentissage automatique pour prédire l'action
      const prediction = await this.learningService.predictAction(sourceWord, bestMatch.word, similarity);

      finalAction = this.determineActionFromScore(similarity.score, prediction.action);
      ...
      if (finalAction === 'merge') {
        ...
        translationGroupId = await this.getOrCreateTranslationGroup(sourceWord, bestMatch.word);
      }
    }
    ...
    if (finalAction === 'uncertain') {
      ...
      return {
        success: false,
        action: 'uncertain',
        message: 'Traduction similaire détectée. Veuillez confirmer votre choix.',
    ...
    sourceWord.availableLanguages = [...new Set(sourceWord.translations.map(t => t.language || 'fr').filter(Boolean))];

    // 5. Mettre à jour les mots-clés extraits si nécessaire
    if (!sourceWord.extractedKeywords || sourceWord.extractedKeywords.length === 0) {
      sourceWord.extractedKeywords = this.similarityService.extractKeywords(sourceWord);
    }
   ...
   */
  async searchTranslationSuggestions(searchDto: SearchTranslationDto): Promise<TranslationSuggestionDto[]> {
    ...
    if (searchDto.searchTerm) {
      const matchingWords = await this.wordModel.find({
      ...
      for (const word of matchingWords) {
        const similarity = this.similarityService.calculateSimilarity(sourceWord, word);
        
        if (similarity.score >= (searchDto.minSimilarity || 0.3)) {
          const prediction = await this.learningService.predictAction(sourceWord, word, similarity);
    ...
    if (suggestions.length < 5 && sourceWord.categoryId) {
      const categoryWords = await this.wordModel.find({
        language: searchDto.targetLanguage,
  ...
  async validateTranslation(
    ...
    const translation = word.translations.find(t => (t as any)._id?.toString() === translationId);
    if (!translation) {
      throw new NotFoundException('Traduction non trouvée');
    }
  ...
  async voteForTranslation(
    ...
    const hasVoted = translation.votedBy?.some(id => id.toString() === userId);
    if (hasVoted) {
      throw new BadRequestException('Vous avez déjà voté pour cette traduction');
    }
  ...
  async getLanguageStats(): Promise<LanguageStatsDto[]> {
    const pipeline = [
      ...
      { $group: {
        _id: '$language',
        totalWords: { $sum: 1 },
        translatedWords: { $sum: { $cond: [{ $gt: ['$translationCount', 0] }, 1, 0] } },
  ...
  private async findSimilarWordsInLanguage(
    ...
    const candidates = await this.wordModel.find({
  ...
  private determineActionFromScore(score: number, predictedAction: string): 'merge' | 'separate' | 'uncertain' {
    if (score > 0.9) return 'merge';
    if (score > 0.6) return 'uncertain';
    return 'separate';
  }

  private async getOrCreateTranslationGroup(word1: Word, word2: Word): Promise<string> {
  ...
  private getSuccessMessage(action: string): string {
    switch (action) {
      case 'merge': return 'Traduction fusionnée automatiquement avec un mot similaire';
      case 'separate': return 'Nouvelle traduction créée';
      default: return 'Traduction traitée';
    }
  }
}