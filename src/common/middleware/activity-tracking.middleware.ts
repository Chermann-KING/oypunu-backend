...
export class ActivityTrackingMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      console.log('🔄 ActivityTrackingMiddleware - Route:', req.method, req.url);
      console.log('👤 User dans request:', user ? `${user.userId || user._id} (${user.username || 'unknown'})` : 'non authentifié');
  ...
  private async updateUserActivity(userId: string): Promise<void> {
    try {
      ...
      if (result) {
        console.log('✅ lastActive mis à jour pour:', result.username, 'à', result.lastActive);
      } else {
