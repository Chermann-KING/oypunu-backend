...
export class HomeService {
  ...
  async getFeaturedWords() {
    ...
    if (shouldUpdateCache || this._featuredWordsCache.length === 0) {
      ...
      await this.updateFeaturedWordsCache();
    }

    return this._featuredWordsCache;
  }

  private async updateFeaturedWordsCache() {
    try {
  ...
  async getStatistics() {
    try {
      ...
      // Compter les langues distinctes parmi les mots approuvés
      const languages = await this._wordModel.distinct('language', {
        status: 'approved',
      });

      console.log(
        `Statistiques: ${activeUsers} utilisateurs, ${definedWords} mots définis, ${languages.length} langues`,
      );
