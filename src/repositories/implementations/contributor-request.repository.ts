 ...
 */
@Injectable()
export class ContributorRequestRepository
  implements IContributorRequestRepository
{
  constructor(
  ...
  async findAll(
    ...
    return DatabaseErrorHandler.handleSearchOperation(async () => {
      ...
      if (typeof isHighPriority === "boolean")
        filter.isHighPriority = isHighPriority;
      if (typeof requiresSpecialReview === "boolean")
        filter.requiresSpecialReview = requiresSpecialReview;
  ...
  async updateSkillsAssessment(
    id: string,
    skills: Record<string, number>
  ): Promise<ContributorRequest | null> {
    return this.update(id, {
      skillsAssessment: new Map(Object.entries(skills)),
    });
  }
  ...
  async addActivityLog(
    ...
    return DatabaseErrorHandler.handleUpdateOperation(
      async () => {
        if (
          !Types.ObjectId.isValid(id) ||
          !Types.ObjectId.isValid(performedBy)
        ) {
          return null;
        }
  ...
  async getActivityLog(id: string): Promise<
    Array<{
      action: string;
      performedBy: string;
      performedAt: Date;
  ...
  async getGlobalStats(): Promise<any> {
    const stats = await this.contributorRequestModel
      .aggregate([
        {
          $group: {
            _id: "$status",
