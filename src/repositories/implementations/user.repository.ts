...
export class UserRepository implements IUserRepository {
  ...
  async updateEmailVerificationToken(id: string, token: string): Promise<boolean> {
    const result = await this.userModel
      .updateOne({ _id: id }, { emailVerificationToken: token })
      .exec();
    return result.modifiedCount > 0;
  }
  ...
  async updatePasswordResetToken(id: string, token: string, expiresAt: Date): Promise<boolean> {
    const result = await this.userModel
      .updateOne({ 
        _id: id 
      }, { 
  ...
  async updateLastActive(id: string, timestamp = new Date()): Promise<boolean> {
    const result = await this.userModel
      .updateOne({ _id: id }, { lastActive: timestamp })
      .exec();
    return result.modifiedCount > 0;
  }
  ...
  async createSocialUser(userData: {
    ...
    const socialUser = new this.userModel({
      ...userData,
      password: '', // Pas de mot de passe pour les utilisateurs sociaux
      isEmailVerified: userData.isEmailVerified || true, // Les comptes sociaux sont généralement vérifiés
      socialProviders: [{
  ...
  async incrementWordCount(id: string): Promise<boolean> {
    const result = await this.userModel
      .updateOne({ _id: id }, { $inc: { wordsAdded: 1 } })
      .exec();
    return result.modifiedCount > 0;
  }
  ...
  async updateUserStats(id: string, stats: {
    ...
    if (stats.wordsAdded !== undefined) {
      updateData.wordsAdded = stats.wordsAdded;
    }
    if (stats.wordsViewed !== undefined) {
      updateData.wordsViewed = stats.wordsViewed;
    }
    if (stats.favoriteWords !== undefined) {
      updateData.favoriteWords = stats.favoriteWords;
    }
    if (stats.streakDays !== undefined) {
      updateData.streakDays = stats.streakDays;
    }
    if (stats.lastStreakDate !== undefined) {
      updateData.lastStreakDate = stats.lastStreakDate;
    }
