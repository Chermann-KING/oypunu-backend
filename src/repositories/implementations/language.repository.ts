...
export class LanguageRepository implements ILanguageRepository {
  ...
  async create(
    ...
    return DatabaseErrorHandler.handleCreateOperation(async () => {
      // Mapper le statut vers systemStatus
      const systemStatus =
        status === "approved"
          ? "active"
          : status === "pending"
  ...
  async findByNameOrCode(criteria: {
    ...
    return DatabaseErrorHandler.handleFindOperation(async () => {
      ...
      if (criteria.nativeName) {
        searchCriteria.push({
          nativeName: new RegExp(`^${criteria.nativeName}$`, "i"),
        });
      }
  ...
  async updateStatus(
    ...
    return DatabaseErrorHandler.handleUpdateOperation(
      async () => {
        ...
        // Mapper les statuts vers systemStatus
  ...
  async countByStatus(): Promise<{
    ...
    return DatabaseErrorHandler.handleSearchOperation(async () => {
      const [pending, approved, rejected] = await Promise.all([
        this.languageModel.countDocuments({ systemStatus: "proposed" }).exec(),
        this.languageModel.countDocuments({ systemStatus: "active" }).exec(),
        this.languageModel
          .countDocuments({ systemStatus: "deprecated" })
          .exec(),
      ]);
  ...
  async countApproved(): Promise<number> {
    return DatabaseErrorHandler.handleSearchOperation(async () => {
      return this.languageModel
        .countDocuments({ systemStatus: "active" })
        .exec();
    }, "Language");
  }
