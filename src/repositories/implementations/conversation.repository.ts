...
export class ConversationRepository implements IConversationRepository {
  ...
  async findByParticipants(participantIds: string[]): Promise<Conversation | null> {
    return DatabaseErrorHandler.handleFindOperation(
      async () => {
        // Valider les IDs
        const validIds = participantIds.filter(id => Types.ObjectId.isValid(id));
  ...
  async searchByTitle(userId: string, query: string, options: {
    limit?: number;
    includeArchived?: boolean;
  } = {}): Promise<Conversation[]> {
    return DatabaseErrorHandler.handleSearchOperation(
  ...
  async addParticipants(conversationId: string, participantIds: string[]): Promise<Conversation | null> {
    return DatabaseErrorHandler.handleUpdateOperation(
      async () => {
  ...
  async removeParticipants(conversationId: string, participantIds: string[]): Promise<Conversation | null> {
    return DatabaseErrorHandler.handleUpdateOperation(
      async () => {
        if (!Types.ObjectId.isValid(conversationId)) {
          return null;
  ...
  async updateLastMessage(conversationId: string, messageId: string, messagePreview: string): Promise<Conversation | null> {
    return DatabaseErrorHandler.handleUpdateOperation(
      async () => {
        if (!Types.ObjectId.isValid(conversationId)) {
          return null;
  ...
  async updateLastActivity(conversationId: string): Promise<Conversation | null> {
    return DatabaseErrorHandler.handleUpdateOperation(
      async () => {
        if (!Types.ObjectId.isValid(conversationId)) {
          return null;
  ...
  async toggleArchive(conversationId: string, userId: string, isArchived: boolean): Promise<Conversation | null> {
    return DatabaseErrorHandler.handleUpdateOperation(
      async () => {
        if (!Types.ObjectId.isValid(conversationId) || !Types.ObjectId.isValid(userId)) {
          return null;
  ...
  async togglePin(conversationId: string, userId: string, isPinned: boolean): Promise<Conversation | null> {
    return DatabaseErrorHandler.handleUpdateOperation(
      async () => {
        if (!Types.ObjectId.isValid(conversationId) || !Types.ObjectId.isValid(userId)) {
          return null;
  ...
  async getMostActive(userId: string, limit: number = 10): Promise<Conversation[]> {
    return DatabaseErrorHandler.handleSearchOperation(
      async () => {
        if (!Types.ObjectId.isValid(userId)) {
          return [];
